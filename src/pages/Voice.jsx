...
import React from 'react';
import { motion } from 'framer-motion';
import { Mic, Headphones, Computer, Building } from 'lucide-react';

import commercialDemo from '../assets/audio/commercial-demo.mp3';
import narrationDemo from '../assets/audio/narration-demo.aac';
import voicePageBg from '../assets/images/voice-page-bg.jpg';
...
const Voice = () => {
  return (
    // We keep the main fixed background container
    <div 
      className="bg-cover bg-fixed bg-center text-light-text"
      style={{ backgroundImage: `url(${voicePageBg})` }}
    >
      <div className="bg-dark-bg/90 backdrop-blur-sm pt-10 pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 space-y-20">
          ...
          <motion.div 
            ...
            transition={{ duration: 1 }}
          >
            <h1 className="text-4xl md:text-6xl font-extrabold text-brand-blue tracking-tight bg-gradient-to-r from-brand-blue to-brand-gold text-transparent bg-clip-text">Professional Voiceover</h1>
            <p className="mt-4 text-lg md:text-xl max-w-2xl mx-auto text-light-text/80">
              A versatile, authentic, and engaging voice ready to bring your project to life.
            </p>
          </motion.div>
          ...
          >
            <h2 className="text-3xl font-bold text-center mb-10 bg-gradient-to-r from-brand-blue to-brand-gold text-transparent bg-clip-text">Listen to My Demos</h2>
            <div className="space-y-6">
              {demos.map((demo, index) => (
                <motion.div 
                  key={index}
                  className="bg-gray-900/50 p-4 rounded-lg shadow-lg" // Kept a slightly darker bg for contrast
                  initial={{ opacity: 0, x: -50 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.5, delay: index * 0.2 }}
